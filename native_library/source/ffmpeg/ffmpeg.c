...
//eoin custom
double avane_speed = 2.0; //fast burst start
...
int avane_get_cancel_transcode() {
...
{
    int i, j;
	av_log(NULL, AV_LOG_INFO, "ff clean up called\n");
    if (do_benchmark) {
	...
	//eoin added these to allow seek
	nb_input_files = 0;
	nb_input_streams = 0;
	nb_output_files = 0;
	nb_output_streams = 0;
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
	...
	int avane_secs = (FFABS(pts) / AV_TIME_BASE);
	int avane_us = (us * 100 / AV_TIME_BASE);
	avaneLogProgress((total_size / 1024.0), avane_secs, avane_us, bitrate, speed, fps, frame_number);
...
static int transcode_init(void)
{
    ...
    for (i = 0; i < nb_input_files; i++) {
        ...
        if (ifile->rate_emu || ifile->rate_emu_out)
...
static int get_input_packet(InputFile *f, AVPacket *pkt)
{
	...
	else if (f->rate_emu_out)
	{
		for (i = 0; i < f->nb_streams; i++) {
			InputStream *ist = input_streams[f->ist_index + i];
			int64_t pts = av_rescale(ist->dts, (1000000 * avane_speed), AV_TIME_BASE);
			int64_t now = av_gettime_relative() - ist->start;
			if (pts > now)
				return AVERROR(EAGAIN);
		}
...
static int transcode(void)
{
	...
	*/
	...
	while (!avane_cancel_transcode) {
		if (avane_pause_transcode == 0) {
	...
	if (avane_cancel_transcode) {
		av_log(NULL, AV_LOG_INFO, "cancelling transcode: %d.\n", avane_cancel_transcode);
	}
...
//init_dynload();
int avane_main_transcode() {
	if (transcode() < 0) return -1;//create another transcode
	return 1;
}
int avane_main_cleanup() {
	ffmpeg_cleanup(0);
	return 1;
}
//main was deleted